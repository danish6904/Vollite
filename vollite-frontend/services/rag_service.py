...
try:
    ...
    print("Warning: LangChain not installed. RAG features will be disabled.")
...
class ForensicsRAGService:
    ...
    def _create_query_from_analysis(self, analysis_data: Dict[str, Any]) -> str:
        ...
        if 'summary' in analysis_data:
            query_parts.append(analysis_data['summary'])
        
        if 'key_findings' in analysis_data:
            query_parts.extend(analysis_data['key_findings'][:3])
    ...
    def _create_analysis_prompt(self, 
        ...
        prompt = f"""
You are a memory forensics expert analyzing a suspicious memory dump.
...
"""
        ...
        if similar_cases:
            ...
            for i, case in enumerate(similar_cases, 1):
                prompt += f"{i}. {case['content'][:200]}...\n"
        
        prompt += """
Based on this information and similar cases, provide:
...
def get_rag_service() -> ForensicsRAGService:
    ...
    if _rag_service_instance is None:
        _rag_service_instance = ForensicsRAGService()
